...
const UserTransactionCard = ({ transaction, index }) => {
    ...
    return (
        <tr className="border-b border-gray-300 hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800">
            ...
            <td className="py-2 px-4 text-[#737373] dark:text-gray-300">{(adminProfit + agentProfit) || fee || 0}</td>
            <td className="py-2 px-4 text-[#737373]">
                <button className={`font-bold text-[1rem] w-full capitalize btn text-white ${status == "accepted" ? 'bg-green-600' : status == "canceled" ? 'bg-red-600' : 'bg-sky-500'}`}>{status || "Accepted"}</button>
            </td>
